...
export function teamImportPreview(manifest: unknown): PendingTeamImport {
  ...
  if (root.format === "handbot.package") return packagePreview(root, manifest);
  if (root.format !== "handbot.team") throw new Error("This is not a BotMRR playbook or legacy HandBot team.");
...
function markdownPackage(markdown: string): unknown {
  ...
  return { format: "handbot.package", version: 1, package: pkg };
}
