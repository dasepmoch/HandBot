...
//
// HandBot owns only the sandbox boundary: image preparation, container
...
export const IMAGE_REPOSITORY = "localhost/handbot/cua-local-vm";
export const IMAGE_LAYER_VERSION = "4";
export const IMAGE_LAYER_LABEL = "com.handbot.image-layer";
export const IMAGE = `${IMAGE_REPOSITORY}:driver-${CUA_DRIVER_VERSION}-v${IMAGE_LAYER_VERSION}`;
export const CONTAINER = "handbot-computer";
export const MANAGED_LABEL = "com.handbot.local-vm";
export const DRIVER_LABEL = "com.handbot.cua-driver";
export const BASE_IMAGE_LABEL = "com.handbot.cua-base";
export const WORKSPACE_LABEL = "com.handbot.workspace";
export const TARGET_LABEL = "com.handbot.local-vm-target";
...
export const CUA_SOCKET = "/run/user/1000/handbot-cua.sock";
export const CUA_EXECUTABLE = "/usr/local/libexec/handbot/cua-driver";
...
RUN printf '%s\\n' \\
      ...
      > /usr/local/bin/prepare-handbot-workspace.sh \\
    && chmod 0755 /usr/local/bin/prepare-handbot-workspace.sh
RUN printf '%s\\n' \\
      '#!/bin/sh' \\
      '/usr/local/bin/prepare-handbot-workspace.sh' \\
      'attempt=0' \\
      ...
      > /usr/local/bin/start-handbot-cua-driver.sh \\
    && chmod 0755 /usr/local/bin/start-handbot-cua-driver.sh
RUN printf '%s\\n' \\
      '' \\
      '[program:handbot-cua-driver]' \\
      'command=/usr/local/bin/start-handbot-cua-driver.sh' \\
      'user=cua' \\
...
function statusProblem(status: ContainerComputerStatus): string | null {
  ...
  if (!status.managed) return "The existing container was not created by HandBot; recreate it";
...
export async function containerComputerStatus(
  ...
  try {
    ...
    // The prepared HandBot derivative has not been built yet.
  }
  ...
  if (canProbe) {
    try {
      ...
      const readinessShot = "/tmp/handbot-readiness.png";
      await runner(
        status.runtime,
        cuaExecArgs([
          "call",
...
async function prepareManagedImage(runtime: Runtime, runner: CommandRunner): Promise<void> {
  ...
  const context = await mkdtemp(join(tmpdir(), "handbot-cua-image-"));
  try {
...
export async function containerComputerScreenshot(
  ...
  try {
    const screenshot = "/tmp/handbot-preview.png";
    await runner(
      status.runtime,
      cuaExecArgs([
        "call",
...
/** Cloud boxes still use HandBot's high-latency REST adapter. Local VMs
