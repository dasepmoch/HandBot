...
const EXCLUDES = `# HandBot checkpoint excludes — never snapshotted, never removed by restore
.git/
.svn/
.hg/
node_modules/
...
function gitEnv(shadow: string, cwd: string): NodeJS.ProcessEnv {
  ...
  env.GIT_AUTHOR_NAME = "HandBot Checkpoint";
  env.GIT_AUTHOR_EMAIL = "checkpoint@handbot.local";
  env.GIT_COMMITTER_NAME = "HandBot Checkpoint";
  env.GIT_COMMITTER_EMAIL = "checkpoint@handbot.local";
  return env;
}
