...
export function codexLocalProviderArgs(
  ...
  if (!host) return [];
  const envKey = `HANDBOT_LOCAL_${host.id.toUpperCase().replace(/[^A-Z0-9]+/g, "_")}_API_KEY`;
...
export async function mergeLocalInject(
  ...
  const probe = env.HANDBOT_PROBE_LOCAL_INJECT ?? process.env.HANDBOT_PROBE_LOCAL_INJECT;
