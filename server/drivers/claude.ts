...
const QUESTION_TIMEOUT_NOTE = "HandBot: nobody answered in time. Use your best judgment and continue.";
const DUPLICATE_ASK_ID_NOTE = "HandBot: duplicate ask id — skipping this request.";
...
function systemEndedReply(kind: Ask["kind"]): { behavior: AskBehavior; message: string } {
  return kind === "question"
    ? { behavior: "answer", message: "HandBot: the turn is ending — wrap up." }
    : { behavior: "deny", message: "HandBot: the turn ended" };
}
  ...
  async create(input: DriverCreateInput<ClaudeConfig>): Promise<ProviderInstance> {
    ...
    const sendTurn = async (turn: SendTurnInput) => {
      ...
      if (turn.integrations?.computer) {
        ...
        // Host tools always route through HandBot's permission broker.
