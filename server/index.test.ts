  ...
  for (;;) {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        if (health.app === "handbot" && health.pid === serverChild.pid && health.static === true) return;
      }
    } catch {
      /* still starting */
    }
...
const storedMessageCount = (threadId: string): number => {
  const db = new DatabaseSync(join(home, ".handbot", "messages.db"), { readOnly: true });
  try {
...
beforeAll(async () => {
  ...
  writeFileSync(join(staticDir, "index.html"), "<!doctype html><title>Packaged HandBot</title>");
  writeFileSync(join(staticDir, "assets", "smoke.css"), "body { color: white; }");
  writeFileSync(
    join(home, ".handbot", "config.json"),
    JSON.stringify({
      instances: {
  ...
  const linkedWorkspace = join(home, ".handbot", "workspaces", "test-bot-a");
  const linkedFile = join(linkedWorkspace, "phone report.md");
  const linkedImage = join(linkedWorkspace, "preview.png");
  const privateAttachments = join(home, ".handbot", "attachments");
  ...
  writeFileSync(
    join(home, ".handbot", "messages-test-linked-file-room-thread.json"),
    JSON.stringify({
  ...
  writeFileSync(
    join(home, ".handbot", "messages-test-goal-restart-thread.json"),
    JSON.stringify({
  ...
  writeFileSync(
    join(home, ".handbot", "routines.json"),
    JSON.stringify({
      version: 1,
      routines: [],
      runs: [{
  ...
  writeFileSync(
    join(home, ".handbot", "messages-test-stranded-room-thread.json"),
    JSON.stringify({
  ...
  writeFileSync(
    join(home, ".handbot", "messages-test-cancel-room-thread.json"),
    JSON.stringify({
...
describe("harness HTTP API", () => {
  it("reconciles durable working goal cards with scheduler truth after a restart", async () => {
    ...
    expect(room.messages.find(
      ...
      text: "Goal failed: HandBot restarted before this goal finished.",
      goalRun: {
        status: "failed",
        detail: "HandBot restarted before this goal finished.",
        turnCount: 2,
  ...
  it("serves packaged UI assets and preserves API 404s", async () => {
    ...
    expect(await root.text()).toContain("Packaged HandBot");
    ...
    expect(await spa.text()).toContain("Packaged HandBot");
  ...
  it("exports every visible bot and imports the team without creating a room", async () => {
    ...
    expect(exported.body).toMatchObject({ format: "handbot.team", version: 2, team: { name: "Field Team" } });
    ...
    expect((await api("POST", "/api/teams/export", {})).body.team.name).toBe("My HandBot Team");
  ...
  it("leaves a failed credential-card continuation on the card without buzzing twice", async () => {
    ...
    try {
      ...
      expect(directCard).toMatchObject({
        kind: "secret",
        text: "Open this conversation in HandBot on your computer to securely enter the OpenAI API key. Credential entry is not available in the mobile app.",
      });
  ...
  it("manages and probes custom MCP servers without returning secret values", async () => {
    ...
    const disk = JSON.parse(readFileSync(join(home, ".handbot", "config.json"), "utf8"));
  ...
  it("mounts the verification skill into a real turn when its trigger appears", async () => {
    ...
    try {
      ...
      expect(system).toContain('<handbot-skill id="create-verification-skill"');
  ...
  it("mounts the verification skill only for the latest channel request", async () => {
    ...
    try {
      ...
      expect(system).toContain('<handbot-skill id="create-verification-skill"');
      expect(system).toContain('<handbot-skill id="phone-harness"');
      ...
      expect(system).not.toContain('<handbot-skill id="create-verification-skill"');
      expect(system).toContain('<handbot-skill id="phone-harness"');
    } finally {
      if (room) {
        expect((await api("POST", `/api/groups/${room.id}/interrupt`, {})).status).toBe(200);
  ...
  it("applies browser disable effects before reporting a removed-profile cleanup failure", async () => {
    ...
    const isolatedData = join(isolatedHome, ".handbot");
          ...
          postMessage(message) {
            if (message?.requestId && /browser-(?:bot|profile)-deleted/.test(message.type ?? "")) {
              queueMicrotask(() => listener?.({ data: {
                type: "handbot:browser-lifecycle-result",
  ...
  it("only enables the exact learned-skill proposal a current client reviewed", async () => {
    ...
    try {
      ...
      writeFileSync(
        join(home, ".handbot", "skill-state", bot.id, "staged.json"),
  ...
  it.skipIf(process.platform === "win32")("stores the credentials file with owner-only permissions", () => {
    expect(statSync(join(home, ".handbot", "config.json")).mode & 0o777).toBe(0o600);
  });
  ...
  it("creates an independent webhook, accepts a delivery, deduplicates it, and rotates its secret", async () => {
    ...
    if (process.platform !== "win32") {
      expect(statSync(join(home, ".handbot", "webhooks.json")).mode & 0o777).toBe(0o600);
    }
  });
...
describe("bot memory API", () => {
  ...
  const workspaceOf = (botId: string) => join(home, ".handbot", "workspaces", botId);
  ...
  it("refuses every coat of path traversal without reading the target", async () => {
    ...
    try {
      ...
      writeFileSync(join(home, ".handbot", "secret.md"), "TOP-SECRET-MARKER sibling");
...
describe("message pages", () => {
  ...
  it("downloads only a file linked by the exact stored bot message", async () => {
    ...
    const linkedFile = join(home, ".handbot", "workspaces", "test-bot-a", "phone report.md");
    ...
    expect((await api(
      ...
      { path: join(home, ".handbot", "workspaces", "test-bot-a", "other.md") },
    )).status).toBe(403);
  ...
  it("downloads an image rendered by the exact stored bot message", async () => {
    const threadId = "test-linked-file-room-thread";
    const linkedImage = join(home, ".handbot", "workspaces", "test-bot-a", "preview.png");
  ...
  it("downloads an exact user attachment only from the private attachment store", async () => {
    const threadId = "test-linked-file-room-thread";
    const shared = join(home, ".handbot", "attachments", "shared-notes.pdf");
    const response = await fetch(
    ...
    expect((await api(
      ...
      { path: join(home, ".handbot", "attachments", "different.pdf") },
    )).status).toBe(403);
    expect((await api(
      "POST",
      `/api/threads/${threadId}/messages/user-outside-file-message/file`,
      { path: join(home, ".handbot", "workspaces", "test-bot-a", "phone report.md") },
    )).status).toBe(403);
  });
