...
const SCHEMA = "aos.handbot_status.v1";
...
export const HANDBOT_STATUS_CACHE_PATH = join(
  homedir(),
  ".local/state/aos-session-bridge/handbot/latest.json",
);

type HandBotSlot = z.output<typeof slotSchema>;
type HandBotUi = z.output<typeof uiSchema>;
type HandBotCapsule = z.output<typeof capsuleSchema>;

export interface HandBotStatusDigest {
...
export interface HandBotStatusReadOptions {
  cachePath?: string;
  now?: Date;
}
...
function expectedUi(twoUp: boolean): HandBotUi {
  return {
    two_up: twoUp,
...
function sameUi(left: HandBotUi, right: HandBotUi): boolean {
  return (
...
function unsignedDocument(capsule: HandBotCapsule): JsonObject {
...
function validSlotState(slot: HandBotSlot): boolean {
  return (
...
function validateCapsule(value: JsonValue): HandBotCapsule | null {
...
function unknownDigest(
  reason: NonNullable<HandBotStatusDigest["reason"]>,
  freshness: HandBotStatusDigest["freshness"] = "unknown",
  receipt?: Pick<HandBotStatusDigest, "observedAt" | "receiptSha256">,
): HandBotStatusDigest {
  return {
    schema: SCHEMA,
    freshness,
...
export function readHandBotStatus(
  options: HandBotStatusReadOptions = {},
): HandBotStatusDigest {
  const raw = privateCache(options.cachePath ?? HANDBOT_STATUS_CACHE_PATH);
  if (raw === null) return unknownDigest("missing_or_insecure");
  let capsule: HandBotCapsule | null;
  try {
  ...
  const result: HandBotStatusDigest = {
    schema: SCHEMA,
    freshness: "fresh",
    ...receipt,
...
export function handBotStatusSystemPrompt(options: HandBotStatusReadOptions = {}): string {
  const status = readHandBotStatus(options);
  const receipt = [
  ...
  const slots = status.slots.length
    ...
    : "none";
  return [
    "TRUSTED HANDBOT STATUS (read-only, validated, no transcript or credential data):",
