...
import { handBotStatusSystemPrompt, readHandBotStatus } from "./handbot-status-capsule.ts";
...
function cachePath(capsule: TestCapsule): string {
  const root = mkdtempSync(join(tmpdir(), "handbot-status-"));
  roots.push(root);
  const parent = join(root, "handbot");
...
posixOnly("readHandBotStatus", () => {
  it("projects only fresh normalized two-VM capability data", () => {
    const capsule = successCapsule();
    // Cross-language receipt produced by scripts/aos_handbot_status.py
    // for this exact normalized fixture.
    expect(capsule.receipt_sha256).toBe(
      "sha256:a0e37282efa7d2932defca1b95d6c7f2f96245007126b6cfdbcbb4aa285ddd58",
    );
    const path = cachePath(capsule);

    const status = readHandBotStatus({ cachePath: path, now: new Date(NOW.getTime() + 1_000) });
    ...
    const prompt = handBotStatusSystemPrompt({ cachePath: path, now: new Date(NOW.getTime() + 1_000) });
  ...
  it("rejects a signed success capsule whose installed and accepted hashes differ", () => {
    ...
    expect(readHandBotStatus({ cachePath: path, now: NOW })).toMatchObject({
      freshness: "unknown",
      reason: "invalid",
  ...
  it("turns stale, future-skewed, failed, and receipt-tampered state into unknown", () => {
    const path = cachePath(successCapsule());
    expect(readHandBotStatus({ cachePath: path, now: new Date("2026-08-22T06:35:00Z") })).toMatchObject({
      freshness: "stale", reason: "stale", runtimeState: "unknown", readyCount: 0,
      ui: { twoUp: false },
    });
    expect(readHandBotStatus({ cachePath: path, now: new Date("2026-08-22T06:29:59Z") })).toMatchObject({
      freshness: "unknown", reason: "clock_skew", runtimeState: "unknown", readyCount: 0,
    });

    const failurePath = cachePath(failedCapsule());
    expect(readHandBotStatus({ cachePath: failurePath, now: new Date(NOW.getTime() + 1_000) })).toMatchObject({
    ...
    expect(readHandBotStatus({ cachePath: tamperedPath, now: new Date(NOW.getTime() + 1_000) })).toMatchObject({
  ...
  it("rejects signed extra fields, insecure modes, and same-path symlinks", () => {
    ...
    expect(readHandBotStatus({ cachePath: extraPath, now: NOW }).reason).toBe("invalid");

    const insecurePath = cachePath(successCapsule());
    chmodSync(insecurePath, 0o644);
    expect(readHandBotStatus({ cachePath: insecurePath, now: NOW }).reason).toBe("missing_or_insecure");

    const targetPath = cachePath(successCapsule());
    const linkPath = join(dirname(targetPath), "latest-link.json");
    symlinkSync(targetPath, linkPath);
    expect(readHandBotStatus({ cachePath: linkPath, now: NOW }).reason).toBe("missing_or_insecure");
  });
  ...
  it("rejects a non-0700 parent and a symlinked parent", () => {
    ...
    expect(readHandBotStatus({ cachePath: looseParentPath, now: NOW }).reason).toBe("missing_or_insecure");

    const targetPath = cachePath(successCapsule());
    const linkRoot = mkdtempSync(join(tmpdir(), "handbot-status-parent-link-"));
    roots.push(linkRoot);
    const linkedParent = join(linkRoot, "handbot");
    symlinkSync(dirname(targetPath), linkedParent, "dir");
    expect(
      readHandBotStatus({ cachePath: join(linkedParent, "latest.json"), now: NOW }).reason,
...
it.skipIf(process.getuid !== undefined)(
  ...
  () => {
    ...
    expect(handBotStatusSystemPrompt({ cachePath: path, now: NOW })).toContain(
      "runtime_state=unknown",
    );
  },
);
