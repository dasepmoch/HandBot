...
export function applyManagedBrokerMessage(message: unknown): boolean {
  ...
  if (
    !parsed.success ||
    parsed.data.type !== "handbot:managed-composio" ||
...
export async function prepareProjectSession(
  ...
  if (trimmed === current?.apiKey && current.sessionId) {
    ...
    if (
      ...
      return {
        apiKey: trimmed,
        userId: existing.config?.user_id ?? current.userId ?? `handbot_${randomUUID()}`,
  ...
  const userId = priorUserId ?? `handbot_${randomUUID()}`;
