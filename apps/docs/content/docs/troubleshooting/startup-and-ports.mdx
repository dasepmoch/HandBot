...
1. Quit every HandBot window.
2. Confirm no old HandBot process remains.
3. Start the app once and wait for the local harness.
4. If it still fails, inspect the desktop log.

On macOS, packaged logs are under `~/Library/Logs/HandBot`. On Windows they are under the HandBot log directory in `%APPDATA%`.
...
Developers can set `OMB_DATA_DIR` to run against a clean HandBot data directory. This is useful for reproduction and screenshots without touching a real workspace.
