...
description: Know what HandBot stores locally and what to preserve.
icon: DatabaseBackup
---

HandBot keeps its application state under `~/.handbot`, including configuration, transcripts, message attachments, device pairing state, and the SQLite message store.

## Back up

Close HandBot before taking a filesystem-level backup so SQLite and configuration files are captured consistently. Preserve the entire `.handbot` directory rather than selecting individual database files.
