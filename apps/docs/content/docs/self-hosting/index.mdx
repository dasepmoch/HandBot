---
title: Self-hosting HandBot
description: Run the harness and desktop app on infrastructure you control.
icon: Server
---

HandBot is local-first by default: the harness listens on loopback, agent CLIs run on your machine, and application state lives under `~/.handbot`.

## Typical self-hosted setup

1. Run HandBot on your personal macOS, Windows, or Ubuntu computer.
...
Self-hosting HandBot does not self-host every optional provider:
