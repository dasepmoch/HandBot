...
```bash
pnpm docs:build
pnpm --filter @handbot/docs types:check
pnpm --filter @handbot/docs lint
```

The changelog reads published releases from `dasepmoch/handbot` plus the
...
Create a second Vercel project beside the existing `handbot.com` project:

1. Import the `dasepmoch/handbot` repository.
...
5. Add `docs.handbot.com` under **Settings → Domains**.
...
preview URLs for documentation pull requests. Keep `handbot.com` on the
