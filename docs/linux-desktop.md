...
HandBot has an Ubuntu 24.04 LTS x86_64 desktop beta. The Electron package embeds the harness server, so
...
- The native Electron window and embedded HandBot server on GNOME Xorg and GNOME Wayland.
...
progress in [issue #29](https://github.com/dasepmoch/handbot/issues/29) and the safety hold in
[issue #345](https://github.com/dasepmoch/handbot/issues/345). Bundled
CUA supply-chain work is tracked in [issue #113](https://github.com/dasepmoch/handbot/issues/113). Xorg is tracked in
[issue #79](https://github.com/dasepmoch/handbot/issues/79), and guarded GNOME/Wayland support in
[issue #109](https://github.com/dasepmoch/handbot/issues/109).

## Download packages

Choose one Ubuntu 24.04 x86_64 package from the latest release:

- [Debian package (`HandBot-amd64.deb`)](https://github.com/dasepmoch/handbot/releases/latest/download/HandBot-amd64.deb) — recommended; APT installs its desktop dependencies.
- [Portable AppImage (`HandBot.AppImage`)](https://github.com/dasepmoch/handbot/releases/latest/download/HandBot.AppImage) — does not install system files.
- [SHA-256 checksums](https://github.com/dasepmoch/handbot/releases/latest/download/SHA256SUMS-ubuntu-x64.txt)

Versioned packages and previous releases remain available on the
[releases page](https://github.com/dasepmoch/handbot/releases).
...
```sh
git clone https://github.com/dasepmoch/handbot.git
cd HandBot
corepack enable
...
- `release/HandBot-<version>-amd64.deb`
- `release/HandBot-<version>-x86_64.AppImage`
...
```sh
sudo apt install ./HandBot-amd64.deb
```

Then open **HandBot** from the GNOME application launcher. To remove it:

```sh
sudo apt remove handbot
```

The portable AppImage does not install system files:

```sh
chmod +x release/HandBot-*-x86_64.AppImage
./release/HandBot-*-x86_64.AppImage
```

For a downloaded release AppImage, use `HandBot.AppImage` in place of the versioned path above.

Application data remains local in `~/.handbot`. Electron browser data and window state use the normal XDG
configuration directory (`~/.config/handbot` unless the environment overrides it).
...
./release/linux-unpacked/handbot
```

## Agent CLI discovery

Applications launched from GNOME do not inherit the same interactive shell `PATH` as a terminal. HandBot
...
```sh
OMB_EXTRA_PATH=/your/custom/bin ./release/HandBot-*-x86_64.AppImage
```

Restart HandBot after installing or signing in to a CLI.
...
24.04 GNOME Wayland session. HandBot detects Wayland before XWayland when both `WAYLAND_DISPLAY` and
...
automatically. HandBot does not capture screen audio, remember the selected monitor after restart, or
...
starts with `--no-overlay`, so Cua's decorative full-screen X11 cursor surface is never created. HandBot also
...
The Xorg runtime uses private sockets, standard permission mode, per-action HandBot approvals,
...
then runs the same smoke against `/opt/HandBot/handbot`. These lanes prove the embedded server and UI are
...
Run the CLI directly in a terminal, finish its sign-in flow, then restart HandBot. If it lives outside the
...
```sh
chmod +x HandBot-*-x86_64.AppImage
file HandBot-*-x86_64.AppImage
```
