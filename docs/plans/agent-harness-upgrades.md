# HandBot harness implementation plan
...
(pi, agent-orchestrator, deepseek-harness) and asking what HandBot lacked by
...
**The reframe that reorders everything.** HandBot is itself a harness. The
...
**On-disk forward compatibility.** `~/.handbot` is shared across app versions.
