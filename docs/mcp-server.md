...
The HandBot desktop app includes a local stdio MCP server. It lets another MCP client coordinate your
HandBot team while the desktop app and its harness are running.
...
Start HandBot, then configure the MCP client to run:
...
```json
{
  "mcpServers": {
    "handbot": {
      "command": "pnpm",
      "args": ["--dir", "/absolute/path/to/HandBot", "mcp"]
    }
  }
}
```
...
```json
{
  "mcpServers": {
    "handbot": {
      "command": "/Applications/HandBot.app/Contents/MacOS/HandBot",
      "args": ["/Applications/HandBot.app/Contents/Resources/server/mcp-server.js"],
...
On Windows, use the installed `HandBot.exe` as `command`, the adjacent
`resources\\server\\mcp-server.js` as the argument, and the same `ELECTRON_RUN_AS_NODE=1` environment value.
The usual per-user install is under `%LOCALAPPDATA%\\Programs\\HandBot`.

On Ubuntu `.deb` installs, the executable is normally `/opt/HandBot/handbot` and the script is
`/opt/HandBot/resources/server/mcp-server.js`. Use the same environment value.

## Connection discovery

With no configuration, the MCP process probes HandBot's three desktop ports (`8799`, `18799`, and `28799`)
and accepts only a health response that identifies itself as HandBot. This handles the desktop's normal
fallback when another local process already owns port 8799.

Set `OMB_PORT` to force one local port, or `HANDBOT_URL` to use an explicit HTTP(S) origin. Cleartext remote
HTTP is rejected unless `ALLOW_INSECURE_HTTP=true`; HTTPS should be used outside loopback. An optional
`HANDBOT_TOKEN` is sent as a bearer token for authenticated reverse proxies. When a token is set, an
explicit `HANDBOT_URL` or `OMB_PORT` is required so the credential is never sent while probing unrelated
local ports. `HANDBOT_MCP_TIMEOUT_MS` can set an HTTP timeout between 1,000 and 120,000 milliseconds.
