...
Cursor is an optional HandBot engine. HandBot runs the official
...
3. Confirm `cursor-agent --version` works. The binary installs to `~/.local/bin` by
   default; HandBot already looks there when launched from a GUI.
...
CLI also implements ACP `session/set_model`, HandBot pins the same id over
...
switch). HandBot still answers ACP `session/request_permission` itself:
