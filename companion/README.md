...
HandBot's harness listens on `127.0.0.1` and nothing else, which is the
...
| `OMB_COMPANION_DIR` | `~/.handbot-companion` | paired devices live here |
...
back to `HandBot` when the harness is not up or has no profile. Read once
