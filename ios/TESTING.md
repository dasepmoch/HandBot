...
```sh
git clone https://github.com/dasepmoch/handbot
cd HandBot
...
dns-sd -B _handbot._tcp                         # macOS: should list the service
```
...
- **The firewall is prompting.** System Settings → Network → Firewall. Incoming
  connections to `node`/HandBot must be allowed, or the phone reaches
...
cd ios && xcodegen generate && open HandBotCompanion.xcodeproj
```
...
```sh
xcodebuild -project HandBotCompanion.xcodeproj -scheme HandBotCompanion \
...
1. **Pair.** In HandBot → Settings → Phone, choose **Pair a phone**.
   Scan the QR code with the phone's Camera, open HandBotMobile,
   ...
   **Connect**. The computer should also appear by name for the manual path:
   ...
   - If the list stays empty, check in this order:
     1. **Local Network permission.** iOS asks once, and a denial is
        permanent and silent. Settings → HandBot → Local Network. If the
...
3. **In HandBot → Settings → Phone:** with Phone access on, the panel now
...
- **Xcode console**, subsystem `com.handbot.companion`: `opening stream`,
