...
enum HandBotSharedKeychain {
    private static let service = "com.handbot.companion.token"
    ...
    static func remove(_ connectionID: String) -> Bool {
        guard let accessGroup = HandBotSharedConfiguration.keychainAccessGroup else {
            return false
        }
    ...
    static func removeIncludingLegacyItem(_ connectionID: String) -> Bool {
        ...
        guard let legacyGroup = HandBotSharedConfiguration.legacyAppKeychainAccessGroup else {
            return false
        }
    ...
    private static func requiredAccessGroup() throws -> String {
        guard let accessGroup = HandBotSharedConfiguration.keychainAccessGroup else {
            throw HandBotSharedKeychainError.configurationMissing
        }
        return accessGroup
    }

    private static func requiredLegacyAccessGroup() throws -> String {
        guard let accessGroup = HandBotSharedConfiguration.legacyAppKeychainAccessGroup else {
            throw HandBotSharedKeychainError.configurationMissing
        }
        return accessGroup
    }
    ...
    private static func save(
        ...
        guard status == errSecSuccess else {
            throw HandBotSharedKeychainError.security(status)
        }
    }
    ...
    private static func readToken(
        ...
        guard let data = item as? Data,
            ...
            throw HandBotSharedKeychainError.security(errSecDecode)
        }
        return token
    }
...
enum HandBotSharedKeychainError: LocalizedError {
