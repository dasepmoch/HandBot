...
enum HandBotConnectionRegistrySource: Equatable {
    case sharedRegistry
...
struct HandBotConnectionRegistryResolution: Equatable {
    let registry: CompanionConnectionRegistry
    let source: HandBotConnectionRegistrySource
}

/// Non-secret pairing metadata shared with extensions.
///
/// Tokens never enter defaults; they live in `HandBotSharedKeychain`. The
...
enum HandBotSharedConnectionStore {
    ...
    static func resolve(
    ...
    ) -> HandBotConnectionRegistryResolution {
        ...
        if let sharedRegistryData,
           let registry = try? decoder.decode(
            ...
            return HandBotConnectionRegistryResolution(
                registry: registry,
    ...
    static func loadRegistry(
        sharedDefaults: UserDefaults? = HandBotSharedConfiguration.sharedDefaults,
