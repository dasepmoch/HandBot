...
- stable `HandBot-amd64.deb` and `HandBot.AppImage` copies used by the latest-download links;
...
[HandBot releases](https://github.com/dasepmoch/handbot/releases). Then verify the checksum
...
Data lives in `~/.handbot/` (bots, transcripts, per-thread NDJSON event logs, config with keys).
...
- **Never touch the real `~/.handbot`.** The setup file points `HOME` at a temp dir; keep it
  that way.
...
API keys are write-only: they land in `~/.handbot/config.json` via `PUT /api/config` and the API
