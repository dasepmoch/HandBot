...
try {
  const status = execFileSync("dpkg-query", ["-W", "-f=${db:Status-Abbrev}", "handbot"], {
    encoding: "utf8",
    stdio: ["ignore", "pipe", "ignore"],
  }).trim();
  if (status.startsWith("ii")) fail("refusing to replace a pre-existing HandBot installation");
} catch (error) {
...
const legacyApp = path.join(legacyRoot, "opt", "HandBot");
const legacyResources = path.join(legacyApp, "resources");
const legacyDeb = path.join(temporary, "handbot_0.1.7_amd64.deb");
...
try {
  ...
  fs.writeFileSync(
    ...
    [
      ...
      "Version: 0.1.7",
      "Architecture: amd64",
      "Maintainer: HandBot CI <ci@handbot.invalid>",
      "Description: Legacy HandBot directory-mode upgrade fixture",
      "",
    ].join("\n"),
    { mode: 0o644 },
  );
  ...
  for (const directory of ["/opt/HandBot", "/opt/HandBot/resources"]) {
  ...
  for (const directory of [
    "/opt/HandBot",
    "/opt/HandBot/resources",
    "/opt/HandBot/resources/cua-linux-x64",
  ]) {
  ...
  for (const executable of ["cua-driver", "cua-cursor-theme"]) {
    const file = path.join("/opt/HandBot/resources/cua-linux-x64", executable);
  ...
  const chromiumSandbox = "/opt/HandBot/chrome-sandbox";
  ...
  const installedVersion = execFileSync("dpkg-query", ["-W", "-f=${Version}", "handbot"], {
    encoding: "utf8",
  }).trim();
  console.log(
