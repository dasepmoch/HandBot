package com.handbot.companion.ui

import com.handbot.companion.core.APIError
import com.handbot.companion.core.Bot
import com.handbot.companion.core.CompanionClient
import com.handbot.companion.core.CompanionEndpointKind
import com.handbot.companion.core.Connection
import com.handbot.companion.core.ConnectionAdvice
import com.handbot.companion.core.Fleet
import com.handbot.companion.core.MessageDestination
import com.handbot.companion.core.Room
import java.net.URI
...
object SharePolicy {
    ...
    fun friendlyMessage(error: Throwable, computerName: String?): String {
        ...
        if (api != null && isAmbiguousTransport(api)) {
            return computerName?.let(::offline) ?: "Couldn't reach your computer. Keep HandBot open and Phone access on, then try again."
        }
        ...
        detail == "Unlock this phone, then try sharing again." ||
            ...
            (
                detail.startsWith("Couldn't reach ") &&
                    detail.endsWith(". Keep HandBot open and Phone access on, then try again.")
                )
    ...
    fun notPaired(): String =
        "Open the HandBot app once after updating. If this phone still isn't connected, pair it before sharing."
    ...
    fun imageSupportUnavailable(): String =
        "Update HandBot on this computer before sharing images."

    fun offline(name: String): String =
        "Couldn't reach $name. Keep HandBot open and Phone access on, then try again."
    ...
    fun unreadable(name: String): String =
        "HandBot couldn't read $name. Try exporting it to Files first."

    fun sendTimedOut(): String = "Sending took too long. Check your connection and try again."

    fun generic(): String = "HandBot couldn't send this. Please try again."

    fun pairingExpired(): String =
        "This phone's pairing has expired. Open HandBot and pair it again."
