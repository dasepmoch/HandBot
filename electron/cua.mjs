...
//    EmbeddedCuaDriverHost so TCC grants attribute to HandBot and the
//    driver inherits them. One prompt, named HandBot, out of the box.
...
const HOST_BUNDLE_ID = "com.handbot.app";
...
async function loadEmbeddedSdk() {
  ...
  process.env.HANDBOT_CUA_SDK_LIBRARY = path.join(
...
async function attachStandalone() {
  ...
  if (!(await socketAlive(STANDALONE_SOCKET))) {
    ...
    // machine already granted — instead of the freshly signed HandBot.
...
async function startEmbedded(binary) {
  ...
  // HandBot rather than to a terminal or helper process.
  ...
  if (!sdk.hasRequiredMacOSPermissions(permissionStatus)) {
    ...
    throw new Error(`${missing || "macOS permissions"} required; grant access in System Settings and restart HandBot`);
  }
...
export async function startCua() {
  ...
  const wantEmbedded =
    app.isPackaged || process.env.HANDBOT_CUA_EMBEDDED === "1";
  let nextConnection;
