...
const BUNDLE = app.isPackaged
  ? path.join(process.resourcesPath, "HandBot Recorder.app")
  : recorderHelperBundle;
...
export function startRecorder(win) {
  ...
  ensureBuilt();
  const sessionDir = mkdtempSync(path.join(app.getPath("temp"), "handbot-recorder-"));
...
export function saveSkillRecording(payload, options = {}) {
  ...
  const dataRoot = options.dataRoot ?? process.env.OMB_DATA_DIR ?? path.join(os.homedir(), ".handbot");
