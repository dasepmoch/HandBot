...
describe("private browser control sync", () => {
  it("mirrors a valid server hold into Electron", () => {
    const take = vi.fn();
    expect(applyBrowserControlHold({ type: "handbot:browser-control", botId: "bot-a", held: true }, take)).toBe(true);
    expect(take).toHaveBeenCalledWith("bot-a");
  });

  it("never treats a generic server release as authority to clear the local gate", () => {
    const take = vi.fn();
    expect(() => applyBrowserControlHold({ type: "handbot:browser-control", botId: "bot-a", held: false }, take))
      .toThrow(/invalid browser-control hold/);
    expect(take).not.toHaveBeenCalled();
  });

  it("rejects malformed bot ids and ignores unrelated private messages", () => {
    expect(() => applyBrowserControlHold({ type: "handbot:browser-control", botId: "../other", held: true }, () => {}))
      .toThrow(/invalid browser-control hold/);
    expect(applyBrowserControlHold({ type: "handbot:browser-connection" }, () => {})).toBe(false);
  });
});
...
describe("private browser lifecycle sync", () => {
  it("accepts exact bot/profile deletion messages", () => {
    expect(decodeBrowserLifecycleMessage({
      type: "handbot:browser-bot-deleted",
      requestId,
      botId: "bot_A-1",
    })).toEqual({ type: "bot-deleted", requestId, botId: "bot_A-1" });
    expect(decodeBrowserLifecycleMessage({
      type: "handbot:browser-profile-deleted",
      requestId,
  ...
  it("rejects malformed lifecycle ids and ignores unrelated messages", () => {
    expect(() => decodeBrowserLifecycleMessage({ type: "handbot:browser-bot-deleted", botId: "../other" }))
      .toThrow(/bot-deleted/);
    expect(() => decodeBrowserLifecycleMessage({ type: "handbot:browser-profile-deleted", partitionId: "work!" }))
      .toThrow(/profile-deleted/);
    expect(() => decodeBrowserLifecycleMessage({ type: "handbot:browser-profile-deleted", partitionId: "guest" }))
      .toThrow(/profile-deleted/);
    expect(() => decodeBrowserLifecycleMessage({
      type: "handbot:browser-profile-deleted",
    ...
    expect(decodeBrowserLifecycleMessage({ type: "handbot:managed-composio" })).toBeNull();
  });
});
